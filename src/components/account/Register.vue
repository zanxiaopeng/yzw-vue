<template>
    <section class="registerbox">
        <div class="login-center font-xiti">
            <p class="input-p" style="display: none;">
                <input type="tel" id="refer" placeholder="推荐人(选填)">
            </p>
            <p class="input-p">
                <input type="tel" v-model="phone" id="account" placeholder="绑定手机号">
            </p>
            <p class="input-p">
                <input type="tel" v-model="smscode" id="ssmscode" placeholder="验证码">
            ...
            <span @click="btnregiste" class="btn-login">立即注册</span>
...
<script>
import { getSmsCode, setRegiste } from '../../service/getData';
    ...
    methods: {
        ...
        async btnregiste() {
            if (!this.phone || !_Common.checkMobile(this.phone)) {
                ...
                const registeData = await setRegiste({
                    data: {
                        username: this.phone,
                ...
                if (registeData.result == 0) {
                    _Common.toastBottom("注册成功,自动跳转中...");
                    let that = this;
                    setTimeout(function () {
                    ...
                    _Common.toastBottom(registeData.message);
                }
            }
        }
    }
